...
class SquidInkStrategy(Strategy):
    ...
    def generate_orders(self, state) -> List[Order]:
        ...
        t = state.market_trades.get('SQUID_INK', [])
        o = state.own_trades.get('SQUID_INK', [])
        # 检查是否有交易数据，并确保t是一个非空列表
        if t and len(t) > 0:
            for i in t:
                if i.buyer == "Olivia":
                    cp = i.price
                    ...
                    for ask_price in sorted(order_depth.sell_orders.keys()):
                        if ask_price <= cp + 10 and available_buy > 0:
                            ask_volume = -order_depth.sell_orders[ask_price]
                            buy_amt = min(ask_volume, available_buy)
                            if buy_amt > 0:
                                orders.append(Order("SQUID_INK", ask_price, math.ceil(buy_amt)))
                                available_buy -= buy_amt
                elif i.seller == "Olivia":
                    cp = i.price
                    # 吃掉所有价格在p-10以内的买单
                    for bid_price in sorted(order_depth.buy_orders.keys(), reverse=True):
                        if bid_price >= cp - 10 and available_sell > 0:
                            bid_volume = order_depth.buy_orders[bid_price]
                            sell_amt = min(bid_volume, available_sell)
                            if sell_amt > 0:
                                orders.append(Order("SQUID_INK", bid_price, -math.ceil(sell_amt)))
                                available_sell -= sell_amt
        ...
        if o and len(o) > 0:
            for i in o:
                if i.buyer == "SUBMISSION":
                    ...
                    # 吃掉所有价格在p+10以内的卖单
                    for ask_price in sorted(order_depth.sell_orders.keys()):
                        if ask_price <= cp + 5 and available_buy > 0:
                            ask_volume = -order_depth.sell_orders[ask_price]
                            buy_amt = min(ask_volume, available_buy)
                            if buy_amt > 0:
                                orders.append(Order("SQUID_INK", ask_price, math.ceil(buy_amt)))
                                available_buy -= buy_amt
                elif i.seller == "SUBMISSION":
                    cp = i.price
                    # 吃掉所有价格在p-10以内的买单
                    for bid_price in sorted(order_depth.buy_orders.keys(), reverse=True):
                        if bid_price >= cp - 5 and available_sell > 0:
                            bid_volume = order_depth.buy_orders[bid_price]
                            sell_amt = min(bid_volume, available_sell)
                            if sell_amt > 0:
                                orders.append(Order("SQUID_INK", bid_price, -math.ceil(sell_amt)))
...
class BasketStrategy(Strategy):
    ...
    def generate_orders_croissant(self, symbol: str, state: TradingState) -> List[Order]:
        ...
        current_position = state.position.get('CROISSANTS', 0)
        available_buy = 250 - current_position
        available_sell = 250 + current_position

        t = state.market_trades.get('CROISSANTS', [])
        o = state.own_trades.get('CROISSANTS', [])
        # 检查是否有交易数据，并确保t是一个非空列表
        if t and len(t) > 0:
            for i in t:
                if i.buyer == "Olivia":
                    ...
                    for ask_price in sorted(order_depth.sell_orders.keys()):
                        if ask_price <= cp + 10 and available_buy > 0:
                            ...
                            buy_amt = min(ask_volume, available_buy)
                            if buy_amt > 0:
                                orders.append(Order("CROISSANTS", ask_price, math.ceil(buy_amt)))
                                available_buy -= buy_amt
                elif i.seller == "Olivia":
                    cp = i.price
                    # 吃掉所有价格在p-10以内的买单
                    for bid_price in sorted(order_depth.buy_orders.keys(), reverse=True):
                        if bid_price >= cp - 10 and available_sell > 0:
                            ...
                            sell_amt = min(bid_volume, available_sell)
                            if sell_amt > 0:
                                orders.append(Order("CROISSANTS", bid_price, -math.ceil(sell_amt)))
                                available_sell -= sell_amt
    ...
    def generate_orders_jams(self, symbol: str, state: TradingState) -> List[Order]:
        ...
        fair_value = self.calculate_fair_value(state.order_depths['JAMS'])

        if fair_value > open_price_jams + 30:
            available_sell = 350 + current_position
            if available_sell > 0:
                sell_amt = min(best_bid_amount, available_sell)
                orders.append(Order('JAMS', best_bid, -math.ceil(sell_amt)))

        elif fair_value < open_price_jams - 28:
            available_buy = 350 - current_position
            if available_buy > 0:
                buy_amt = min(-best_ask_amount, available_buy)
                orders.append(Order('JAMS', best_ask, math.ceil(buy_amt)))

        return orders

    def generate_orders_djembes(self, symbol: str, state: TradingState) -> List[Order]:
        open_price_dj = 13409.5
        orders = []
        order_depth = state.order_depths['DJEMBES']
        current_position = state.position.get('DJEMBES', 0)
        ...
        fair_value = self.calculate_fair_value(state.order_depths['DJEMBES'])

        if fair_value > open_price_dj and self.dj_time <= 5:
            self.dj_time += 1
            available_sell = 60 + current_position
            if available_sell > 0:
                sell_amt = min(best_bid_amount, available_sell)
                orders.append(Order('DJEMBES', best_bid, -math.ceil(sell_amt)))

        elif fair_value > open_price_dj + 15:
            available_sell = 60 + current_position
            if available_sell > 0:
                sell_amt = min(best_bid_amount, available_sell)
                orders.append(Order('DJEMBES', best_bid, -math.ceil(sell_amt)))

        elif fair_value < open_price_dj - 50:
            available_buy = 50 - current_position
            if available_buy > 0:
                buy_amt = min(-best_ask_amount, available_buy)
                orders.append(Order('DJEMBES', best_ask, math.ceil(buy_amt)))
...
class VolcanicRockStrategy(Strategy):
    ...
    def generate_orders(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        current_position = state.position.get("VOLCANIC_ROCK", 0)
        ...
        if fair_value > self.openprice + 85:
            available_sell = 400 + current_position
            if available_sell > 0:
                sell_amt = min(best_bid_amount, round(0.8*available_sell))
                orders["VOLCANIC_ROCK"] = [Order("VOLCANIC_ROCK", best_bid, -math.floor(sell_amt))]
                return orders

        elif fair_value < self.openprice - 80:
            available_buy = 400 - current_position
            if available_buy > 0:
                buy_amt = min(-best_ask_amount, round(0.8*available_buy))
                orders["VOLCANIC_ROCK"] = [Order("VOLCANIC_ROCK", best_ask, math.floor(buy_amt))]
                return orders
