...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if not state.traderData or state.traderData.strip() == "":
            ...
            try:
                ...
                logger.print(f"Warning: Failed to decode traderData: {state.traderData}")
                trader_data = {}

        logger.print("traderData (Loaded):", json.dumps(trader_data))
        ...
        for product in state.order_depths:
            logger.print(f'------------Trading {product}------------')
            if product == 'KELP':
    ...
    def trade(self, state, product, profit_pct_limit, position_limit, historical_prices):
        ...
        #打印订单簿
        logger.print(f'Product {product}, Buy orders: {buy_orders}, Sell orders: {sell_orders}')
        orders: List[Order] = []
        position = state.position.get(product, 0)
        logger.print(f'Product {product}, Position: {position}')
        ...
        if product == 'RAINFOREST_RESIN':
            fair_price = 10000
        logger.print(f"Product {product}, Fair price : " + str(fair_price))
        logger.print(f"Product {product}, Expect bid : " + str(expect_bid))
        logger.print(f"Product {product}, Expect ask : " + str(expect_ask))
        #根据持仓、动量预测、订单簿不平衡度调整fair price
        alpha = 0
        beta = 0
        gamma = 0
        logger.print(f'Product {product}, Position: {position}')
        logger.print(product, type(product))
        momentum = self.price_momentum(historical_prices, product)
        logger.print(f'Product {product}, Price momentum: {momentum:.2f}')
        obi = self.orderbook_imbalance(state, product, fair_price)
        logger.print(f'Product {product}, Orderbook imbalance: {obi:.2f}')
        fair_price = fair_price + (alpha * position + beta * momentum + gamma * obi)
        logger.print(f'Product {product}, Adjusted fair price: {fair_price:.2f}, Alpha {alpha:.2f}, Beta {beta:.2f}, Gamma {gamma:.2f}')
        ...
        while i >= 0 and j >= 0:
            ...
            logger.print(f'Dealing with depth {i}, {j}, ask_price: {ask_price}, ask_amount: {ask_amount}, bid_price: {bid_price}, bid_amount: {bid_amount}')

            if ask_amount == 0:
                logger.print(f'Product {product}, Ask amount at depth {i} has been fully filled. Skipping')
                i -= 1
                continue

            if bid_amount == 0:
                logger.print(f'Product {product}, Bid amount at depth {j} has been fully filled. Skipping')
                j -= 1
                continue
            
            #logger.print(f'Product {product}, depth of ask order: {i}, price: {ask_price}, amount: {ask_amount}')
            #logger.print(f'Product {product}, depth of bid order: {j}, price: {bid_price}, amount: {bid_amount}')
            
            #主动交易，此处待引入头寸控制机制
            #ask_price + 1小于fair_price，直接买入
            if ask_price + 1 < fair_price:
                logger.print(f'Product {product}. Asking price is lower than fair price, price: {ask_price}, fair_price: {fair_price}')
                #最大可以买入的amount
            ...
            if bid_price - 1 > fair_price:
                logger.print(f'Product {product}. Bidding price is higher than fair price, price: {bid_price}, fair_price: {fair_price}')
                #最大可卖出（做空）的amount
            ...
            if ask_price < expect_ask:
                logger.print(f'Asking price too low, price: {ask_price}, expect_ask: {expect_ask}')
                i -= 1
                continue
            if bid_price > expect_bid:
                logger.print(f'Bidding price too high, price: {bid_price}, expect_bid: {expect_bid}')
                j -= 1
                continue'
            '''
            ...
            if spread / fair_price > profit_pct_limit:
                logger.print(f'Product {product}. Spread is profitable, spread_pct: {spread/fair_price*100:.2f}')
                #清仓机制，待改进为软硬清仓
                if position > 0:  
                    # 如果当前持仓为正，优先卖出
                    logger.print(f'Product {product}, Current position is negative {position}, buying to close position')
                    #只卖出到平仓
                    ...
                    logger.print(f'Selling product {product}, selling {sell_amount} at {ask_price}, Current position {position}')
                elif position < 0:
                    # 如果当前持仓为负，优先买入平仓
                    logger.print(f'Product {product}, Current position is negative {position}, buying to close position')
                    ...
                    position += buy_amount
                    logger.print(f'Buying product {product}, buying {buy_amount} at {bid_price}, Current position {position}')
                else:
                    # 如果没有持仓，同时买入和卖出
                    amount = min(-ask_amount, bid_amount)
                    amount = min(amount, position_limit) #仓位限制
                    logger.print(f'Product {product}, No position, executing market making')
                    ...
                    logger.print(f'Executing market making: Product {product}, Buy {amount} at {bid_price}, Sell {amount} at {ask_price}')
            #如果spread不够，则直接跳出（以后可以根据更多的条件确认要不要吃下这些订单， 比如跟expect bid和expect ask比较）
            else:
                logger.print(f'Product {product}. Spread is not profitable, spread_pct: {spread/fair_price*100:.2f}, exit loop')
                break
            if i <= 0 and j <= 0:
                logger.print(f'Product {product}, Reach orderbook edge, i: {i}, j: {j}, exit loop')
                break
    ...
    def estimate_fair_price(self, state: TradingState, product: str) -> int:
        ...
        if order_depth.buy_orders and order_depth.sell_orders:
            ...
            #logger.print('Using order depth to estimate fair price')
            ...
            if product == 'KELP':
                logger.print(f'Product {product} has no order depth. Using default price of 2025')
                return 2025,1 , 1
            if product == 'RAINFOREST_RESIN':
                logger.print(f'Product {product} has no order depth. Using default price of 10000')
                return 10000,1, 1 
    ...
    def calculate_ema(self, prices: List[float], span: int, product: str) -> float:
        ...
        logger.print(f'Product {product}, EMA({span}) = {ema:.2f}')
        return ema

    def price_momentum(self, historical_prices: List[int], product: str) -> float:
        '''计算价格动量'''
        logger.print(f'Product {product}, historical prices:', historical_prices[-20:])
