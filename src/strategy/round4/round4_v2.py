...
class MacaronStrategy(Strategy):
    ...
    def __init__(self,
                 ...
                 # ✨ NEW parameters — feel free to tweak | 新增参数，可灵活调整
        ...
        # --- NEW: store the tuning parameters | 存储调节参数 ---
    ...
    def calculate_dynamic_threshold(self):
        if len(self.fair_value_history) < 2:
            return 4.0  # 默认值
        price_changes = [self.fair_value_history[i] - self.fair_value_history[i-1] 
                        for i in range(1, len(self.fair_value_history))]
        volatility = np.std(price_changes)
        return max(2.0, 3 * volatility)  # 至少保留2的安全边际
    ...
    def should_convert(self, state: TradingState, conversion_type: str) -> bool:
        ...
        if conversion_type == "BUY" and position < self.position_limit:
            ...
            return buy_cost < best_ask - dynamic_threshold
            
        elif conversion_type == "SELL" and position > -self.position_limit:
            # 卖给Pristine Cuisine是否有利可图
            return sell_revenue > best_bid + dynamic_threshold
    ...
    def determine_optimal_conversions(self, state: TradingState) -> int:
        ...
        if self.should_convert(state, "BUY"):
            # 计算最大可买入量
            buy_cost = obs.askPrice + obs.transportFees + obs.importTariff
            profit_per_unit = market_fair_value - buy_cost - self.storage_cost
    ...
    def generate_orders(self, state: TradingState) -> List[Order]:
        ...
        # 仓位调整系数 - 仓位越大，卖出意愿越强
        position_factor = 0.2 * position / self.position_limit
        ...
        if (state.observations
            ...
            sunlight_bonus = max(0,
                                 obs.sunlightIndex - self.sun_threshold) * self.sun_coeff  # <<< updated

            # High sugar price raises cost | 糖价越高，成本越高
            sugar_penalty = max(0,
                                obs.sugarPrice - self.sugar_threshold) * self.sugar_coeff  # <<< updated
