...
class SquidInkStrategy(Strategy):
    def __init__(self, symbol: str, position_limit: int, time_window, int = 200, ma_window: int = 20,
        ...
        self.fair_value_ma_history = deque(maxlen= ma_window)
    ...
    def calculate_fair_price(self, order_depth: OrderDepth) -> float:
        """计算中间价"""
    ...
    def generate_orders(self, state) -> List[Order]:
        ...
        if self.current_mode == "market_making":
            if len(self.fair_value_ma20_history) < self.ma_window or abs(fair_value - self.fair_value_ma20_history[-1]) <= self.band_width:
                #如果数据长度不足或者价差在band_width之内，采用中性（做市策略）
                orders = []
                # 获取当前市场数据
                order_depth = state.order_depths[self.symbol]
                current_position = state.position.get(self.symbol, 0)
                ...
                stdev = np.std(list(self.fair_value_history)[:120]) #计算120个数据标准差
    ...
    def load_state(self, state):
        ...
        #保存ma20
        fair_value_ma20 = self.calculator.calculate_ma(list(self.fair_value_history), self.ma_window) if len(self.fair_value_history) >= self.ma_window else 0
        self.fair_value_ma20_history.append(fair_value_ma20)
        if len(self.fair_value_ma20_history) > self.ma_window:
            self.fair_value_ma20_history.popleft()
