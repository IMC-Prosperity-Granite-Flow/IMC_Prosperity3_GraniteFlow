...
class SquidInkStrategy(Strategy):
    ...
    def __init__(self, symbol: str, position_limit: int, timewindow: int = 20):
        super().__init__(symbol, position_limit)

        self.timewindow = timewindow
        # 历史数据（如果需要使用的话）
        self.history = deque(maxlen=timewindow)
    ...
    def calculate_moving_average(self, values: List[float]) -> float:
        """计算简单的移动平均"""
        if len(values) == 0:
            return 0
    ...
    def calculate_standard_deviation(self, values: List[float]) -> float:
        ...
        if len(values) < 2:
            return 0
        mean = self.calculate_moving_average(values)
