...
class SquidInkStrategy(Strategy):
    def __init__(self, symbol: str, position_limit: int, ma_window: int = 200,
                ...
                trend_window: int = 100, take_spread: float = 10, alpha: float = -0.03):
        super().__init__(symbol, position_limit)
    ...
    def generate_orders(self, state) -> List[Order]:
        ...
        if self.current_mode == "market_making":
            if len(self.fair_value_ma200_history) < 200 or abs(fair_value - self.fair_value_ma200_history[-1]) <= self.band_width:
                ...
                # 处理卖单（asks）的限价单
                for ask_price, ask_volume in sorted(sell_orders):
                    if ask_price < adjusted_fair_value and ask_price != best_ask:
                ...
                # 处理买单（bids）的限价单
                for bid_price, bid_volume in sorted(buy_orders, reverse=True):
                    if bid_price > adjusted_fair_value and bid_price != best_bid:
                ...
                self.breakout_timestamp = self.timestamp
                #反向吃满
        ...
        # Strategy 2: Breakout Trend Following
            ...
            if position * self.direction < self.position_limit:
                #继续下单
                if self.direction == 1:
                    #突破是向上的，先做多
                    max_amount = min(best_ask_amount, self.position_limit - position)
            ...
            # 回归就清仓
            if abs(fair_value - self.breakout_price) < vol_10 * 0.5 and self.timestamp - self.breakout_timestamp > 500:
                logger.print(f"Fall back! {fair_value}")
                self.current_mode = "Closing position"
            
        #清仓模式
        elif self.current_mode == "Closing position":
            if position == 0:
                ...
                logger.print(f"Closing position {position}")
                if self.direction == 1:
                #突破是向上的，平空
