    ...
    def generate_basket_orders(self, state: TradingState) -> List[Order]:
        ...
        for basket in ['PICNIC_BASKET1', 'PICNIC_BASKET2']:
            if basket in price_diffs:
                ...
                # 动态调整阈值基于相关性
                base_threshold = self.std_thresholds[basket]
                adjusted_threshold = base_threshold

                # 相关性高时降低阈值，相关性低时提高阈值
                if ewma_corr > 0.9:
                    adjusted_threshold = max(1.5, base_threshold * 0.8)  # 相关性高，降低阈值更积极交易
                elif ewma_corr < 0.85:
                    adjusted_threshold = base_threshold * 1.2  # 相关性低，提高阈值更保守交易
                ...
                if buy_signal and max_buy_volume > 0 and basket in state.order_depths:
                    ...
                    for price, volume in sell_orders:
                        ...
                        if buyable > 0:
                            ...
                            logger.print(
                                f"BUY {basket}: {buyable} @ {price} (相关性: {ewma_corr:.2f}, 阈值: {adjusted_threshold:.2f})")
                ...
                # 执行卖出
                    ...
                    for price, volume in buy_orders:
                        ...
                        if sellable > 0:
                            ...
                            logger.print(
                                f"SELL {basket}: {sellable} @ {price} (相关性: {ewma_corr:.2f}, 阈值: {adjusted_threshold:.2f})")
...
class VolcanicRockStrategy(Strategy):
    def __init__(self, symbol: str, position_limit: int):
        ...
        self.rock_fair_value_history = deque(maxlen=120)
        self.ma_120 = 0
    ...
    def calculate_time_value(self, voucher_symbol: str, ma_120, voucher_price: float) -> float:
        config = self.voucher_config.get(voucher_symbol)
        return config["strike"] + voucher_price - ma_120 if config else 0.0

    def determine_ATM(self, ma_120) -> str:
        if ma_120 >= 10300:
    ...
    def generate_orders(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        if len(self.rock_fair_value_history) > 120:
            self.rock_fair_value_history.popleft()
        if len(self.rock_fair_value_history) >= 120:
            self.ma_120 = np.mean(list(self.rock_fair_value_history)[-120:])
        else:
            self.ma_120 = S
            return orders

        # 确定当前ATM
        current_atm = self.determine_ATM(self.ma_120)
        atm_order_depth = state.order_depths.get(current_atm, OrderDepth())
        atm_price = self.calculate_mid_price(atm_order_depth)
        if atm_price == 0:
            return orders
        time_value = self.calculate_time_value(current_atm, self.ma_120, atm_price)
        ...
        # 如果当前是Abnormal模式
            ...
            if self.active_atm == current_atm:
                atm_depth = state.order_depths.get(current_atm, OrderDepth())
                atm_price = self.calculate_mid_price(atm_depth)
                current_tv = self.calculate_time_value(current_atm, self.ma_120, atm_price)
                # 继续买入标的
                if current_tv > self.active_time_value:
                    qty = (self.position_limit - rock_position) // 2
                    if qty > 0 and rock_order_depth.sell_orders:
                        best_ask = min(rock_order_depth.sell_orders.keys())
                        orders["VOLCANIC_ROCK"] = [Order("VOLCANIC_ROCK", best_ask, math.floor(qty))]
                    ...
                    for symbol in self.voucher_config:
                        ...
                        if self.voucher_config[symbol]["strike"] < active_strike:
                            ...
                            pos_limit = self.voucher_config[symbol]["position_limit"]  # 正确获取方式
                            current_pos = state.position.get(symbol, 0)
                            buy_qty = (pos_limit - current_pos) // 2
                            if buy_qty > 0:
                                if symbol not in orders:
                                    orders[symbol] = []
                                orders[symbol].append(Order(symbol, best_ask, round(buy_qty)))
                ...
                # 平仓逻辑
                    ...
                    if rock_position != 0:
                        if rock_order_depth.buy_orders:
                            ...
                            if available_sell > 0:
                                ...
                                orders["VOLCANIC_ROCK"] = [Order("VOLCANIC_ROCK", best_bid, -math.ceil(sell_qty))]

                    # 平仓所有凭证
                    total_position = abs(rock_position)  # 初始化总持仓计算
                    for symbol in self.voucher_config:
                        current_pos = state.position.get(symbol, 0)
                    ...
                    # 严格检查所有仓位
                    if total_position < 20:
                        self.current_mode = "Normal"
                        self.active_atm = None
                        self.active_time_value = 0
                    return orders

            if self.active_atm != current_atm and self.active_atm is not None:
                if self.voucher_config[current_atm]["strike"] < self.voucher_config[self.active_atm]["strike"]:
                    ...
                    if rock_position != 0:
                        if rock_order_depth.buy_orders:
                            ...
                            available_sell = self.position_limit + rock_position
                            if available_sell > 0:
                                sell_qty = min(best_bid_amount, 0.2*available_sell)
                                orders["VOLCANIC_ROCK"] = [Order("VOLCANIC_ROCK", best_bid, -math.ceil(sell_qty))]
