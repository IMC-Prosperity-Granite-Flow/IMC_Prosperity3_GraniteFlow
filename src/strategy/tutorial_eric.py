...
class Trader:
    ...
    def amethyst_orders(self, order_depth: OrderDepth, fair_value: int, width: int, position: int, position_limit: int) -> List[Order]:
        ...
        ask_candidates = [price for price in order_depth.sell_orders.keys() if price > fair_value]
        baaf = min(ask_candidates) if ask_candidates else fair_value + 1

        bid_candidates = [price for price in order_depth.buy_orders.keys() if price < fair_value]
        bbbf = max(bid_candidates) if bid_candidates else fair_value - 1
    ...
    def run(self, state: TradingState):
        ...
        if "KELP" in state.order_depths:
            amethyst_position = state.position["KELP"] if "KELP" in state.position else 0
            amethyst_orders = self.amethyst_orders(state.order_depths["KELP"], amethyst_fair_value, amethyst_width, amethyst_position, amethyst_position_limit)
            result["KELP"] = amethyst_orders
