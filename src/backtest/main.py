...
def extract_submission_trade_history_log(file_path: str) -> pd.DataFrame:
    """
    从日志文件中提取 Trade History 部分的内容（假定以 JSON 格式存储），
    并过滤出 buyer 或 seller 为 "SUBMISSION" 的记录。
    """
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    trade_idx = content.find("Trade History:")
    if trade_idx == -1:
        ...
        return pd.DataFrame()
    # 提取 Trade History 部分文本（从 "Trade History:" 到文件末尾）
    trade_text = content[trade_idx + len("Trade History:"):].strip()
    if not trade_text:
        print("提取到的 'Trade History' 文本为空")
        return pd.DataFrame()
    try:
        # 假定 trade_text 为 JSON 数组格式
        trade_data = json.loads(trade_text)
    except Exception as e:
        print("解析 Trade History JSON 时发生错误:", e)
        return pd.DataFrame()
    ...
    def _build_order_depths(self, group: pd.DataFrame) -> Dict[str, OrderDepth]:
        ...
        for _, row in group.iterrows():
            ...
            for level in range(1, 4):
              ...
              if bid_price_col in row and not pd.isna(row[bid_price_col]) and bid_volume_col in row and not pd.isna(
                    row[bid_volume_col]):
                  od.buy_orders[int(row[bid_price_col])] = int(row[bid_volume_col])
              if ask_price_col in row and not pd.isna(row[ask_price_col]) and ask_volume_col in row and not pd.isna(
                    row[ask_volume_col]):
                # 注意卖单数量通常以负数表示
                od.sell_orders[int(row[ask_price_col])] = int(row[ask_volume_col]) * -1
...
if __name__ == "__main__":
    ...
    real_trades = extract_submission_trade_history_log(log_file)
    df_backtest = pd.DataFrame(trades)
    df_real = pd.DataFrame(real_trades)
    df_backtest_sorted = df_backtest.sort_values(by="timestamp")
    df_real_sorted = df_real.sort_values(by="timestamp")

    # 使用 merge_asof 按最接近的时间戳对齐（允许 50 单位内的差异，可根据需要调整 tolerance 参数）
    df_aligned = pd.merge_asof(df_backtest_sorted, df_real_sorted, on="timestamp",
                               direction="nearest", tolerance=50,
                               suffixes=('_backtest', '_real'))
    df_aligned.sort_values(by="timestamp", inplace=True)
    # 先进行 merge_asof 得到 df_aligned 后，再进行替换
    df_aligned['buyer_backtest'] = df_aligned['buyer_backtest'].replace({
    'MARKET': '',
    'MY_SIDE': 'SUBMISSION'})
    df_aligned['seller_backtest'] = df_aligned['seller_backtest'].replace({
    'MARKET': '',
    'MY_SIDE': 'SUBMISSION'})

    df_aligned['buyer_real'] = df_aligned['buyer_real'].replace({'MARKET': '','MY_SIDE': 'SUBMISSION',  # 如果真实记录里也可能出现 MY_SIDE，可以加这一行
    'SUBMISSION': 'SUBMISSION'})
    df_aligned['seller_real'] = df_aligned['seller_real'].replace({
    'MARKET': '',
    'MY_SIDE': 'SUBMISSION',
    'SUBMISSION': 'SUBMISSION'
})

    print("\n=== 按 timestamp 对齐后的对比结果（处理后） ===")
    df_aligned.to_csv('data/backtest/backtest_result.csv')
